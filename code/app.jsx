...
function isAlienTeam(p) { return p.role === "alien" || p.infected; }
...
const EVT_COLOR = {
  infect:"#fb923c", infect_immune:"#22d3ee", heal:"#60a5fa", false_heal:"#f97316",
  kill:"#f87171", group_kill:"#f87171", empath_scan:"#a78bfa", vote:"#facc15",
};

function fmtPlayer(players, id, infected) {
  const p = players[id]; if (!p) return `?${id}`;
...
function fmtEvent(e, players) {
  ...
  switch (e.type) {
    case "infect":        return `  ${p(e.from,false)} infected ${p(e.to,false)}`;
    case "infect_immune": return `  ${p(e.from,false)} tried ${p(e.to,false)} (immune)`;
    case "heal":          return `  ${p(e.from,false)} healed ${p(e.to,false)}`;
    case "false_heal":    return `  ${p(e.to,true)} lied to ${p(e.from,false)}${e.framed!=null?` → framed ${p(e.framed,false)}`:""}`;
    case "empath_scan":   return `  ${p(e.from,e.fromInfected)} scanned ${p(e.to,e.toInfected)} → ${e.revealed}`;
    case "kill":          return `  ${p(e.from,e.fromInfected)} killed ${p(e.to,e.toInfected)}`;
    case "group_kill":    return `  ${p(e.killers[0],e.killersInfected?.[0])}+${p(e.killers[1],e.killersInfected?.[1])} → killed ${p(e.victim,e.victimInfected)}`;
    case "vote":          return e.elimId!=null ? `  vote: ${p(e.elimId,e.elimInfected)} out (${e.elimRole})` : "  vote: tie";
    default: return null;
  }
}
...
function Sigil({ role, color }) {
  ...
  if (role === "human") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      ...
      <circle cx="45" cy="45" r="36" stroke={c} strokeWidth="0.5" strokeDasharray="3 6" opacity="0.2"/>
    </svg>
  );
  if (role === "doctor") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      <rect x="38" y="20" width="14" height="50" rx="3" stroke={c} strokeWidth="1.5" fill={c} fillOpacity="0.1"/>
      <rect x="20" y="38" width="50" height="14" rx="3" stroke={c} strokeWidth="1.5" fill={c} fillOpacity="0.1"/>
      <circle cx="45" cy="45" r="32" stroke={c} strokeWidth="0.5" strokeDasharray="2 5" opacity="0.25"/>
    </svg>
  );
  if (role === "alien") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      <ellipse cx="45" cy="36" rx="20" ry="16" stroke={c} strokeWidth="1.5"/>
      <ellipse cx="45" cy="36" rx="7" ry="10" stroke={c} strokeWidth="1" fill={c} opacity="0.1"/>
      <circle cx="37" cy="32" r="3.5" fill={c} opacity="0.7"/>
      <circle cx="53" cy="32" r="3.5" fill={c} opacity="0.7"/>
      <path d="M28 52 Q36 66 45 64 Q54 66 62 52" stroke={c} strokeWidth="1.2"/>
      <path d="M21 48 Q14 38 17 27" stroke={c} strokeWidth="1" strokeDasharray="2 3" opacity="0.5"/>
      <path d="M69 48 Q76 38 73 27" stroke={c} strokeWidth="1" strokeDasharray="2 3" opacity="0.5"/>
    </svg>
  );
  if (role === "immune") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      <path d="M45 14 L70 28 L70 52 Q70 70 45 78 Q20 70 20 52 L20 28 Z" stroke={c} strokeWidth="1.5"/>
      <path d="M45 24 L62 34 L62 52 Q62 64 45 70 Q28 64 28 52 L28 34 Z" stroke={c} strokeWidth="0.7" fill={c} fillOpacity="0.05"/>
      <path d="M33 46 L41 54 L57 38" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
  if (role === "duelist") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      <line x1="22" y1="22" x2="68" y2="68" stroke={c} strokeWidth="1.8" strokeLinecap="round"/>
      <line x1="68" y1="22" x2="22" y2="68" stroke={c} strokeWidth="1.8" strokeLinecap="round"/>
  ...
  if (role === "empath") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      <ellipse cx="45" cy="45" rx="22" ry="14" stroke={c} strokeWidth="1.5"/>
      <circle cx="45" cy="45" r="6" stroke={c} strokeWidth="1.2" fill={c} fillOpacity="0.2"/>
      <circle cx="45" cy="45" r="2.5" fill={c} opacity="0.8"/>
      {[0,30,60,90,120,150,180,210,240,270,300,330].map((a,i) => (
        <line key={i}
          x1={45+Math.cos(a*Math.PI/180)*28} y1={45+Math.sin(a*Math.PI/180)*28}
          x2={45+Math.cos(a*Math.PI/180)*36} y2={45+Math.sin(a*Math.PI/180)*36}
          stroke={c} strokeWidth="0.9" strokeLinecap="round" opacity="0.5"/>
      ))}
    </svg>
  );
  if (role === "predisposed") return (
    <svg viewBox="0 0 90 90" fill="none" style={s}>
      <path d="M45 15 A30 30 0 0 1 75 45" stroke={c} strokeWidth="1.5" strokeLinecap="round"/>
      <path d="M75 45 A30 30 0 0 1 45 75" stroke={c} strokeWidth="1.5" strokeLinecap="round"/>
      <path d="M45 75 A30 30 0 0 1 15 45" stroke={c} strokeWidth="1.5" strokeLinecap="round"/>
      <path d="M15 45 A30 30 0 0 1 45 15" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeDasharray="4 4"/>
      <path d="M45 45 L55 28" stroke={c} strokeWidth="1.2" strokeLinecap="round" opacity="0.8"/>
      <path d="M45 45 L65 52" stroke={c} strokeWidth="1" strokeLinecap="round" opacity="0.6"/>
      <path d="M45 45 L38 65" stroke={c} strokeWidth="1.2" strokeLinecap="round" opacity="0.7"/>
      <path d="M45 45 L22 38" stroke={c} strokeWidth="0.8" strokeLinecap="round" opacity="0.5"/>
      <circle cx="56" cy="26" r="1.5" fill={c} opacity="0.6"/>
      <circle cx="37" cy="67" r="1.5" fill={c} opacity="0.5"/>
      <circle cx="45" cy="45" r="4" stroke={c} strokeWidth="1" fill={c} fillOpacity="0.2"/>
    </svg>
  );
  return null;
}
...
function PlayerCard({ p, highlight }) {
  ...
  return (
    <div style={{width:38,height:44,opacity:p.alive?1:0.2,position:"relative",
      ...
      {infected && <span className="absolute top-0.5 right-0.5 w-1.5 h-1.5 rounded-full bg-orange-500 z-10"/>}
      {!p.alive && <span className="absolute inset-0 flex items-center justify-center text-gray-700 text-lg font-bold z-10">✕</span>}
    </div>
  );
}
...
function EventLog({ log, lastEvent, players }) {
  ...
  useEffect(() => { if (ref.current) ref.current.scrollTop = ref.current.scrollHeight; }, [log, lastEvent]);
  const lastStr = lastEvent ? JSON.stringify(lastEvent) : null;
  return (
    <div ref={ref} className="h-32 overflow-y-auto text-xs font-mono bg-black/40 rounded p-2">
      {log.map((entry, i) => (
        <div key={i}>
          <div style={{borderTop:"1px solid #1f2937",margin:"2px 0"}}/>
          {entry.events.map((e, j) => {
            const msg = fmtEvent(e, players); if (!msg) return null;
            const isLast = lastStr && JSON.stringify(e) === lastStr;
            return (
              <div key={j} style={{color:EVT_COLOR[e.type]||"#9ca3af",
                fontWeight:isLast?"bold":"normal", background:isLast?"rgba(255,255,255,0.05)":"none"}}>
                {msg}
              </div>
            );
          })}
...
function VisualizerPanel({ cfg }) {
  ...
  useEffect(() => { setHistory([createGame(cfg)]); setPlaying(false); }, [cfg]);
  useEffect(() => {
    if (!playing || game.winner) return;
    const t = setTimeout(() => setHistory(h => [...h, stepGameEvent(h[h.length-1])]), speed);
    return () => clearTimeout(t);
  }, [playing, game.winner, game.tick, game.pendingEvents?.length, speed]);

  const stepFwd = () => { setPlaying(false); if (!game.winner) setHistory(h => [...h, stepGameEvent(h[h.length-1])]); };
  const stepBck = () => { setPlaying(false); setHistory(h => h.length > 1 ? h.slice(0,-1) : h); };
  const reset   = () => { setHistory([createGame(cfg)]); setPlaying(false); };
  ...
  if (game.lastEvent) {
    ...
    [e.from, e.to, e.victim, e.elimId].filter(x=>x!=null).forEach(id=>hl.add(id));
    (e.killers||[]).forEach(id=>hl.add(id));
  }

  const statusOf = p => p.alive ? (isAlienTeam(p)?"alive_infected":"alive_healthy") : (isAlienTeam(p)?"dead_infected":"dead_healthy");
  const ORDER = ["alive_healthy","alive_infected","dead_healthy","dead_infected"];
  const sorted = ORDER.flatMap(s => game.players.filter(p => statusOf(p)===s));
  const GBG = {alive_healthy:"#0d1f0d",alive_infected:"#1f0a00",dead_healthy:"#141414",dead_infected:"#1a0f00"};
  const GFG = {alive_healthy:"#4ade80",alive_infected:"#fb923c",dead_healthy:"#6b7280",dead_infected:"#b45309"};
  const GLBL = {alive_healthy:"alive · human",alive_infected:"alive · alien",dead_healthy:"dead · was human",dead_infected:"dead · was alien"};
  const SEP = "#374151"; const CW=34,CH=30;

  const byRoom = new Map(game.rooms.map(r=>[r.id,[]]));
  for (const p of game.players) if (p.alive && p.roomId!=null && byRoom.has(p.roomId)) byRoom.get(p.roomId).push(p);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-3 text-xs font-mono flex-wrap">
        ...
        {(game.pendingEvents?.length>0) && <span className="text-gray-600">+{game.pendingEvents.length}</span>}
        <span className="text-red-400">aliens {alienTeam}</span>
        <span className="text-blue-400">humans {humanTeam}</span>
        {game.winner && (
          <span className={`font-bold ${game.winner==="aliens"?"text-red-300":game.winner==="tie"?"text-yellow-300":"text-green-300"}`}>
            {game.winner==="tie"?"→ tie":`→ ${game.winner} win`}
          </span>
        )}
        <div className="ml-auto">
          <select value={speed} onChange={e=>setSpeed(+e.target.value)} className="bg-gray-800 text-white rounded px-1 py-0.5 text-xs">
            <option value={1200}>1x</option><option value={500}>2x</option>
            <option value={150}>8x</option><option value={30}>30x</option>
          </select>
        </div>
      </div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        {game.rooms.map(room => {
          ...
          return (
            <div key={room.id}
              style={{width:3*38+2*2+2*6,padding:6,flexShrink:0,boxSizing:"border-box",
              ...
              <div style={{display:"flex",gap:2,minHeight:44}}>
                {ps.map(p => <PlayerCard key={p.id} p={p} highlight={hl.has(p.id)}/>)}
              </div>
            </div>
          );
        })}
      ...
      <div style={{overflowX:"auto",border:`1px solid ${SEP}`,borderRadius:6}}>
        <table style={{borderCollapse:"collapse",tableLayout:"fixed",fontSize:9,fontFamily:"monospace"}}>
          <thead>
            <tr style={{backgroundColor:"#111827",position:"sticky",top:0,zIndex:10}}>
              <th style={{width:60,textAlign:"right",paddingRight:6,color:"#374151",fontWeight:"normal",
                fontSize:8,position:"sticky",left:0,backgroundColor:"#111827",zIndex:11}}>
                down / right
              </th>
              {sorted.map((p,ci) => {
                ...
                const ui = ROLE_DEFS[p.role]?.ui ?? ROLE_DEFS.human.ui;
                return (
                  <th key={p.id} style={{width:CW,fontWeight:"normal",textAlign:"center",
                    borderLeft:prev&&prev!==statusOf(p)?`2px solid ${SEP}`:"none",paddingBottom:2,
                    boxShadow:hl.has(p.id)?`inset 0 -2px 0 ${ui.color}`:"none"}}>
                    <div style={{color:ui.color,opacity:p.alive?1:0.4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      {ui.letter}{p.name.match(/_(\d+)$/)?.[1]??""}
                    </div>
                    <div style={{color:GFG[statusOf(p)],fontSize:7,opacity:p.alive?1:0.6}}>
                ...
                <React.Fragment key={row.id}>
                  {groupStart && (
                    <tr style={{backgroundColor:"#111827"}}>
                      <td colSpan={sorted.length+1} style={{padding:"2px 6px",fontSize:8,
                        color:GFG[statusOf(row)],borderTop:ri>0?`2px solid ${SEP}`:"none",fontFamily:"monospace"}}>
                        {GLBL[statusOf(row)]}
                      </td>
                    </tr>
                  )}
                  <tr style={{backgroundColor:GBG[statusOf(row)],
                    boxShadow:hl.has(row.id)?`inset 3px 0 0 ${rowUi.color}`:"none"}}>
                    <td style={{textAlign:"right",paddingRight:6,whiteSpace:"nowrap",
                      color:rowUi.color,opacity:row.alive?1:0.5,
                      position:"sticky",left:0,zIndex:5,backgroundColor:GBG[statusOf(row)],fontSize:9}}>
                      {rowUi.letter}{row.name.match(/_(\d+)$/)?.[1]??""}
                      <span style={{color:GFG[statusOf(row)],marginLeft:2,fontSize:7}}>
                        {!row.alive?"x":isAlienTeam(row)?"inf":"ok"}
                      </span>
                    </td>
                    {sorted.map((col, ci) => {
                      const prevCol = ci>0 ? statusOf(sorted[ci-1]) : null;
                      const bl = prevCol&&prevCol!==statusOf(col)?`2px solid ${SEP}`:"none";
                      if (col.id === row.id) return (
                        <td key={col.id} style={{width:CW,height:CH,borderLeft:bl,textAlign:"center",color:"#374151",verticalAlign:"middle"}}>-</td>
                      );
                      ...
                      const bg = fb===null?"transparent":fb>0.7?"#4c1010":fb>0.4?"#4a2200":fb>0.1?"#222":"transparent";
                      const fg = fb===null?"#6b7280":fb>0.7?"#fca5a5":fb>0.4?"#fde68a":fb>0.1?"#e5e7eb":"#9ca3af";
                      return (
                        <td key={col.id} style={{width:CW,height:CH,borderLeft:bl,backgroundColor:bg,
                          textAlign:"center",verticalAlign:"middle",padding:"0 1px"}}>
                          <div style={{color:fg,lineHeight:1.2,fontSize:9}}>
                            {pct!==null?`${pct}%`:"."}
                            {rc>=1&&<span style={{color:rb==="alien"?"#f87171":"#4ade80"}}> v</span>}
                          </div>
                          <div style={{fontSize:7,lineHeight:1.1,color:rc>=1?"#c4b5fd":"#9ca3af",overflow:"hidden",whiteSpace:"nowrap"}}>
                            {rb??"\u00a0"}
                          </div>
                        </td>
                      );
      ...
      <div className="flex gap-2">
        {[
          ...
          {label:"<",      fn:stepBck, cls:"bg-gray-700 hover:bg-gray-600", disabled:history.length<=1},
          {label:">",      fn:stepFwd, cls:"bg-gray-700 hover:bg-gray-600", disabled:!!game.winner},
          {label:playing?"Pause":"Play", fn:()=>setPlaying(x=>!x),
           cls:playing?"bg-amber-800 hover:bg-amber-700":"bg-green-800 hover:bg-green-700", disabled:!!game.winner},
        ].map(({label,fn,cls,disabled}) => (
          <button key={label} onClick={fn} disabled={disabled}
            className={`px-3 py-1 text-xs text-white rounded ${cls} disabled:opacity-30`}>{label}</button>
        ))}
      </div>

      <EventLog log={game.log} lastEvent={game.lastEvent} players={game.players}/>
    </div>
  );
}
...
function ConfigPanel({ cfg, onChange }) {
  const set = (k, v) => {
    v = +v; const next = {...cfg, [k]:v};
    if (k!=="nPlayers"&&k!=="nRooms") {
      const minP = next.nAliens+next.nDoctors+next.nDuelists+next.nImmune+next.nEmpaths+next.nPredisposed+1;
      if (next.nPlayers<=minP) next.nPlayers=minP+1;
    }
    next.nRooms = Math.max(next.nRooms, Math.ceil(next.nPlayers/3));
    onChange(next);
  };
  const nHumans = cfg.nPlayers-cfg.nAliens-cfg.nDoctors-(cfg.nDuelists||0)-(cfg.nImmune||0)-(cfg.nEmpaths||0)-(cfg.nPredisposed||0);
  ...
  const fields = [
    ...
    {label:"Aliens",k:"nAliens",min:0,max:6},{label:"Doctors",k:"nDoctors",min:0,max:6},
    {label:"Duelists",k:"nDuelists",min:0,max:4},{label:"Immune",k:"nImmune",min:0,max:4},
    {label:"Empaths",k:"nEmpaths",min:0,max:4},{label:"Predisposed",k:"nPredisposed",min:0,max:4},
  ];
  const roleDesc = {
    alien:"alien · infects in 1v1", doctor:"doctor · heals in 1v1 · learns infector",
    human:"crew · gossips · calls votes", duelist:"duelist · 1 kill when belief >55%",
    ...
    <div className="space-y-4">
      {fields.map(({label,k,min,max}) => (
        <div key={k} className="flex items-center gap-3">
          ...
          <input type="range" min={min} max={max} value={cfg[k]??min}
            onChange={e=>set(k,e.target.value)} className="flex-1 accent-indigo-500"/>
          <span className="w-5 text-right text-white font-mono text-sm">{cfg[k]}</span>
        </div>
      ))}
      <div className="flex items-center gap-3">
        <span className="text-gray-400 text-xs w-36 shrink-0">Vote aggressiveness</span>
        <input type="range" min={0} max={threshSteps.length-1}
          value={threshSteps.indexOf(cfg.voteThreshold??0.7)}
          onChange={e=>onChange({...cfg,voteThreshold:threshSteps[+e.target.value]})}
          className="flex-1 accent-indigo-500"/>
        <span className="w-24 text-right text-white font-mono text-xs">{threshLabel[cfg.voteThreshold]??cfg.voteThreshold}</span>
      </div>
      <p className="text-xs text-gray-500">{Math.max(0,nHumans)} plain crew · max 3/room · reveals in 1v1 only</p>
      <div className="text-xs text-gray-600 space-y-1 border-t border-gray-800 pt-3">
        {Object.entries(ROLE_DEFS).map(([role,def]) => (
          <p key={role}>
            <span style={{color:def.ui.color}} className="font-bold">{def.ui.letter}</span>{" "}
            {roleDesc[role]}
          </p>
        ))}
        <p className="text-gray-700 mt-1">3-person rooms: gossip only · alien-team gossip lies</p>
      </div>
      <div className="border-t border-gray-800 pt-3 space-y-2">
        <div className="flex gap-2 items-center">
          <span className="text-gray-500 text-xs">Config JSON</span>
          <button onClick={()=>{try{onChange({...DEFAULT_CFG,...JSON.parse(document.getElementById("cfg-json").value)});}catch{}}}
            className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 text-white rounded">Apply</button>
        </div>
        <textarea id="cfg-json" key={JSON.stringify(cfg)} defaultValue={JSON.stringify(cfg)}
...
function MonteCarloPanel({ cfg }) {
  ...
  const run = useCallback(() => {
    ...
    setTimeout(() => { setResults(runMonteCarlo(cfg,n)); setRunning(false); }, 20);
  }, [cfg,n]);
  const pct = v => results ? ((v/results.total)*100).toFixed(1)+"%" : "—";
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <span className="text-gray-400 text-sm">Runs</span>
        <select value={n} onChange={e=>setN(+e.target.value)} className="bg-gray-800 text-white text-sm rounded px-2 py-1">
      ...
      {results && (
        <>
          <div className="grid grid-cols-2 gap-2 text-center">
            {[{label:"Human wins",v:results.humanWins,color:"#3b82f6"},{label:"Alien wins",v:results.alienWins,color:"#ef4444"},
              {label:"Ties",v:results.ties,color:"#facc15"},{label:"Timeout",v:results.timeouts,color:"#6b7280"}
            ].map(({label,v,color}) => (
              <div key={label} className="bg-gray-800 rounded-lg p-3">
                <div className="text-xl font-bold" style={{color}}>{pct(v)}</div>
                <div className="text-gray-400 text-xs">{label}</div>
                <div className="text-gray-600 text-[10px]">{v}/{results.total}</div>
              </div>
            ))}
          </div>
          <p className="text-gray-400 text-sm">Avg: <span className="text-white font-mono">{results.avgTicks.toFixed(1)}</span> ticks</p>
          <ResponsiveContainer width="100%" height={80}>
            <BarChart data={[{name:"Humans",w:results.humanWins},{name:"Aliens",w:results.alienWins},{name:"Ties",w:results.ties},{name:"T/O",w:results.timeouts}].filter(d=>d.w>0)} margin={{top:0,right:0,bottom:0,left:-20}}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151"/>
              <XAxis dataKey="name" tick={{fill:"#9ca3af",fontSize:11}}/><YAxis tick={{fill:"#9ca3af",fontSize:10}}/>
              <Tooltip contentStyle={{backgroundColor:"#1f2937",border:"none",color:"#fff"}}/>
              <Bar dataKey="w" fill="#6366f1" radius={[3,3,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
          <ResponsiveContainer width="100%" height={80}>
            <BarChart data={results.distArr} margin={{top:0,right:0,bottom:0,left:-20}}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151"/>
              <XAxis dataKey="tick" tick={{fill:"#9ca3af",fontSize:10}}/><YAxis tick={{fill:"#9ca3af",fontSize:10}}/>
              <Tooltip contentStyle={{backgroundColor:"#1f2937",border:"none",color:"#fff"}}/>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4 font-sans max-w-xl mx-auto">
      ...
      <div className="flex gap-1 mb-3 bg-gray-900 p-1 rounded-lg">
        {[{id:"config",label:"Config"},{id:"visualize",label:"Visualize"},{id:"montecarlo",label:"Monte Carlo"}].map(t => (
